var scorebook = function (data) {
  ...
  function pushWicket(batsmanId, howOut, fielderId) {
    ...
    // Duck check.
    var valid = ((d.wickets.length > 0) && (d.balls.length > 0) ? d.wickets[d.wickets.length - 1].ballId !== d.balls.length : true) === false ? false :
        (/^(bowled|lbw|handled the ball|hit wicket|hit the ball twice)$/).test(howOut) ? "bowler" :
